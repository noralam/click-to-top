...
if( get_option('besdismiss1') || $theme_domain == 'beshop' || $theme_domain == 'beshop-pro' ){
    return;
}
$class = 'eye-notice notice notice-warning is-dismissible';

$message = __( '<strong>BeShop is one of the best WordPress theme for your shop and blog websites. Beshop Pro version has huge Options. Try free version now !!! You can also buy pro version for one click checkout options, Products Quick view, Wishlist, Products Tabs, Products animation slider, Products grid, Advanced products details offar options, Blog grid, Blog list and many more.</strong>', 'beshop' );
$url1 = esc_url('https://wpthemespace.com/product/beshop/');
$url3 =esc_url('https://wpthemespace.com/product/beshop/?add-to-cart=2897');

printf( '<div class="%1$s" style="padding:10px 15px 20px;"><p>%2$s</p><a target="_blank" class="button button-primary" href="%3$s" style="margin-right:10px">'.__('Beshop Pro Details','beshop').'</a><a target="_blank" class="button button-primary" href="%4$s" style="margin-right:10px">'.__('Upgrade Pro','beshop').'</a><button class="nothanks link btnend" style="margin-left:10px">'.__('Dismiss the notice','beshop').'</button></div>', esc_attr( $class ), wp_kses_post( $message ),$url1,$url3 ); 
}
add_action( 'admin_notices', 'clicktop_new_optins_texts' );
...
function wpspace_admin_notice_option(){
    if(date('D') == 'Sun' && get_option('clickitem54') ) { 
        delete_option( 'clickitem554' );
    }

    if(isset($_GET['cdismissed']) && $_GET['cdismissed'] == 1 ){
        update_option( 'clickitem554', 1);
    }
    if(isset($_GET['besdismiss']) && $_GET['besdismiss'] == 1 ){
        update_option( 'besdismiss1', 1);
    }
}
